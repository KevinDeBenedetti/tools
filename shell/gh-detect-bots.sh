#!/usr/bin/env bash
# gh-detect-bots.sh — Detect (and optionally purge) bot commits in a Git repository
# Usage: ./gh-detect-bots.sh [--repo <owner/repo>] [--local] [--dry-run] [--purge-bots] [--format <text|json>]
...
Examples:
  ...
  # Scan a remote repo
  ./gh-detect-bots.sh --repo owner/repo

  # Output as JSON
  ./gh-detect-bots.sh --format json

  # Remove bot commits from local repo
  ./gh-detect-bots.sh --purge-bots

  # Dry-run purge (preview only)
  ./gh-detect-bots.sh --purge-bots --dry-run
EOF
  exit 0
}
