...
# Tests for purge-release.sh
# Uses mocked gh/jq commands — never touches real repos

setup() {
  DIR="$(cd "$(dirname "$BATS_TEST_FILENAME")" && pwd)"
  REPO_ROOT="$(cd "$DIR/../.." && pwd)"
  SCRIPT="${REPO_ROOT}/shell/github/purge-release.sh"
...
MOCK
  ...
  run "$SCRIPT" --repo owner/repo --dry-run --keep-latest 1
  assert_success
...
MOCK
  ...
  run "$SCRIPT" --repo owner/repo --dry-run --tag-pattern "*-beta"
  assert_success
  refute_output --partial "v2.0.0"
